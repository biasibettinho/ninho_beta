...
const THEMES = {
  pink: { bg: '#FFF1F2', accent: '#F472B6', soft: '#FBCFE8' },
  lavender: { bg: '#F5F3FF', accent: '#A78BFA', soft: '#DDD6FE' },
  mint: { bg: '#F0FDF4', accent: '#34D399', soft: '#A7F3D0' },
  sunset: { bg: '#FFF7ED', accent: '#FB923C', soft: '#FED7AA' }
};
...
const App: React.FC = () => {
  ...
  // Efeito de Login Forçado ao Abrir
  useEffect(() => {
  ...
  // Função de Logout Segura para evitar erro de 404 no histórico do navegador
  const handleLogout = () => {
  ...
  const handleLogin = async (email: string) => {
    ...
    if (!cleanEmail.includes('@')) return alert("Por favor, digite um e-mail válido.");
    setIsLoading(true);
    try {
      const loggedUser = await dbService.login(cleanEmail);
      if (!loggedUser) {
        setIsNewUser(true);
        setUser(null);
      } else {
        setUser(loggedUser);
        localStorage.setItem('paz_no_ninho_user_email', cleanEmail);
  ...
  const startPaymentFlow = async () => {
    ...
    if (!email) return alert("E-mail não identificado.");
    setIsGeneratingPayment(true);
    try {
      const res = await dbService.generatePayment(email, selectedPlan);
      setPaymentData(res);
  ...
  const currentTheme = THEMES[couple?.theme || 'pink'] || THEMES.pink;
  const themeStyles = {
    '--theme-bg': currentTheme.bg,
    '--theme-accent': currentTheme.accent,
    '--theme-accent-soft': currentTheme.soft
  ...
  if (isLoading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-pink-50">
      ...
      <p className="mt-4 text-pink-500 font-black text-xl animate-pulse font-cute text-center px-6">Abrindo o ninho para você...</p>
    </div>
  );

  // TELAS DE LOGIN / ONBOARDING (FORÇADAS)
  if (!user || !couple) {
    if (!user && !isNewUser) {
      return (
        <div className="min-h-screen flex items-center justify-center p-6 bg-pink-50">
          <div className="w-full max-w-md soft-ui-card p-12 text-center border-4 border-pink-100 animate-in zoom-in duration-500">
            ...
            <p className="text-pink-600 font-black mb-10 uppercase tracking-[0.3em] text-[10px] bg-pink-50 py-2 rounded-full">Seu refúgio de harmonia</p>
            <input 
              type="email" placeholder="seuemail@ninho.com" 
              className="w-full p-6 bg-white border-4 border-pink-100 rounded-[2.5rem] mb-6 text-center font-black text-lg text-gray-700 outline-none focus:ring-8 ring-pink-100 transition-all"
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-b from-blue-50 to-white">
        <div className="w-full max-w-md soft-ui-card p-10 text-center border-4 border-blue-100 shadow-2xl relative">
          ...
          <h2 className="text-4xl font-black mb-1 text-gray-800 font-cute">Falta pouco! ✨</h2>
          <p className="text-gray-400 font-bold mb-10 text-sm truncate">{emailInput}</p>
          ...
          <div className="relative mb-10">
            ...
            <div className="relative flex justify-center text-xs"><span className="px-4 bg-white text-blue-300 font-black uppercase tracking-widest text-[10px]">Ou entre em um ninho existente</span></div>
          </div>
          ...
          <button onClick={async () => {
            try {
              ...
              if(!c) alert('Código não encontrado! 🥺 Verifique se o código está correto.');
            } catch (e) { alert("Erro ao conectar ao ninho."); }
          }} className="w-full bg-gray-800 text-white font-black py-6 rounded-[2.5rem] hover:bg-black transition-all shadow-lg mb-8">CONECTAR AGORA</button>
          
          <button onClick={handleLogout} className="text-gray-400 font-black text-[10px] uppercase tracking-widest hover:text-pink-500 transition-colors flex items-center justify-center gap-2 mx-auto">
             <LogOut className="w-4 h-4" /> Não é você? Voltar ao Início
          </button>
        </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-12">
                {Object.keys(THEMES).map(t => (
                  <button key={t} onClick={async () => { 
                    ...
                    <div className="w-full h-3 rounded-full mb-3" style={{ backgroundColor: THEMES[t as keyof typeof THEMES].accent }}></div>
                    {t}
                  </button>
                ))}
              </div>
